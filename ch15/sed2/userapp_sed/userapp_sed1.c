...
static void encrypt_msg(int fd, char *msg, char *prg)
{
	 ...
	 * adta in the usual fashion... Instead, the user must first write data
	 * to the driver via the 'encrypt' ioctl, and subsequently - upon checking
	 * that the data is indeed processed - issue another 'retrieve' ioctl to
	 * obtain the encrypted message in suer space.
	 */
