/*
 * ch10/7_lockdep_eg/fixed_lockdep/thrd_showall_fixed.c
 ***************************************************************
 ...
 ****************************************************************
 * Brief Description:
 * This kernel module is based upon our earlier kernel module
 *  ch10/7_lockdep_eg/buggy_lockdep/thrd_showall_buggy.c. 
 * However, when we refactored it to use the get_task_comm() routine to
 * retrieve the name of the thread, it's buggy!
 * The bug turns out to be a recursive locking issue, detected by lockdep.
 * Here, we fix this by first unlocking the relevant lock, then calling
 * get_task_comm() and then re-locking it.
 *
...
MODULE_DESCRIPTION("LKDC book: ch10/7_lockdep_eg/fixed_lockdep:"
...
"--------------------------------------------------------------------------------\n";
	...
	/* In the 'buggy' ver, LOCKDEP did catch a deadlock here !! (at the
	 * point that get_task_comm() was invoked).
	 * the reason: get_task_comm() attempts to take the very same lock
 	 * that we just took above: task_lock(t);  !! This is obvious
	 * self-deadlock... ; so, we fix it here by first unlocking it,
	 * calling get_task_comm(), and then re-locking it.
	 */
		task_unlock(t);
